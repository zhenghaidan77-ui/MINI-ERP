...
async function createProposal() {
    ...
    // Updated Rows based on CJ Freshway
    ...
    for(let i = 2; i <= 8; i++) {
        ...
        if(i === 7 || i === 8) {
            sheet.getRow(i).getCell(2).font = { bold: true, color: { argb: 'FF0000FF' } }; // Blue text
            sheet.getRow(i).getCell(3).font = { bold: true, color: { argb: 'FF0000FF' } };
        }
    }

    await workbook.xlsx.writeFile('해천굴소스_로켓배송_입점제안서_CJ반영본.xlsx');
    console.log('생성 완료');
}
