...
async function createProposal() {
    ...
    for(let i = 2; i <= 8; i++) {
        ...
        if(i === 5 || i === 7 || i === 8) { // Highlight the corrected pricing and CJ changes
            sheet.getRow(i).getCell(2).font = { bold: true, color: { argb: 'FF0000FF' } }; 
            sheet.getRow(i).getCell(3).font = { bold: true, color: { argb: 'FF0000FF' } };
        }
    }

    const desktopPath = path.join(os.homedir(), 'Desktop', '해천굴소스_로켓배송_입점제안서_최종본_업데이트.xlsx');
    await workbook.xlsx.writeFile(desktopPath);
    console.log('생성 완료: ' + desktopPath);
}
